...
FORCE_INLINE void _draw_heater_status(const uint8_t x, const int8_t heater, const bool blink)
{
  ...
  _draw_centered_temp(0.5f + (thermalManager.degTargetHotend(heater)), x, 7);

  _draw_centered_temp(0.5f + (thermalManager.degHotend(heater)), x, 28);

  const uint8_t h = isBed ? 7 : HEAT_INDICATOR_X,
                y = isBed ? 18 : 17;
  if (thermalManager.isHeatingHotend(heater))
  {
