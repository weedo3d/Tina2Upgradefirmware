...
void GCodeQueue::get_serial_commands() {
  ...
  while (length < BUFSIZE && MSerial1.available()) 
  {
    ...
    if (inp0.State() == PS_START && !wtvar_uart0_binmode)
    {
        if (ISEOL(serial_char)) 
        {
          ...
          if (npos) 
          {
            ...
            if (gcode_N != last_N + 1 && !M110)
              return gcode_line_error(PSTR(MSG_ERR_LINE_NO), 0);
            ...
            if (apos) 
            {
              ...
              if (strtol(apos + 1, nullptr, 10) != checksum)
                return gcode_line_error(PSTR(MSG_ERR_CHECKSUM_MISMATCH), 0);
            }
            else
              return gcode_line_error(PSTR(MSG_ERR_NO_CHECKSUM), 0);
