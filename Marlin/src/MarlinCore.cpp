  ...
  inline void finishSDPrinting() {
    ...
    switch (card.sdprinting_done_state) {
      ...
      case 4:                                   // Display "Click to Continue..."
        #if 0 //  HAS_RESUME_CONTINUE                 // 30 min timeout with LCD, 1 min without
      ...
      case 5:
        ...
        SERIAL_ECHOLNPGM_0(MSG_FILE_PRINTED);
...
void setup() {
  ...
  SERIAL_ECHOLNPGM_0("start");
  SERIAL_ECHO_START();

  // Check startup - does nothing if bootloader sets MCUSR to 0
  byte mcu = HAL_get_reset_source();
  if (mcu &  1) SERIAL_ECHOLNPGM_0(MSG_POWERUP);
  if (mcu &  2) SERIAL_ECHOLNPGM_0(MSG_EXTERNAL_RESET);
  if (mcu &  4) SERIAL_ECHOLNPGM_0(MSG_BROWNOUT_RESET);
  if (mcu &  8) SERIAL_ECHOLNPGM_0(MSG_WATCHDOG_RESET);
  if (mcu & 32) SERIAL_ECHOLNPGM_0(MSG_SOFTWARE_RESET);
  HAL_clear_reset_source();

  serialprintPGM_0(MSG_MARLIN);
  SERIAL_CHAR_0(' ');
  SERIAL_ECHOLNPGM_0(SHORT_BUILD_VERSION);
  SERIAL_EOL_0();

  SERIAL_ECHO_START();
  SERIAL_ECHOLNPAIR_0(MSG_FREE_MEMORY, freeMemory(), MSG_PLANNER_BUFFER_BYTES, (int)sizeof(block_t) * (BLOCK_BUFFER_SIZE));
  ...
  strcpy_P(ipAddress, PSTR(MSG_WIFINOTCONNECTED));		// perron reset the ip address
